...
def sendAttack():
    ...
    requests.post('http://localhost:5000/addevent', json = {"User": userObject.__dict__, "DetectionPoint" : dp1.__dict__, "Time" : datetime.now().isoformat()})
    requests.post('http://localhost:5000/addevent', json = {"User": userObject.__dict__, "DetectionPoint" : dp2.__dict__, "Time" : datetime.now().isoformat()})
    requests.post('http://localhost:5000/addevent', json = {"User": userObject.__dict__, "DetectionPoint" : dp3.__dict__, "Time" : datetime.now().isoformat()})
    time.sleep(1)
    checkResp = requests.get('http://localhost:5000/getResponses')
    print (checkResp.text)
    if '{"Username": "repTest", "Detection Point": "Multiple", "SessionID": "xxxx", "Response": "Logout"}' in checkResp.text:
